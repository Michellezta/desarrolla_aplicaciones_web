<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Productos</title>
    <link rel="stylesheet" href="css/bootstrap.css">
    <script src="js/jquery-3.6.1.js"></script>
</head>
<body>
    <?php include 'menu.php' ?>
    <div class="container">
        <div class="row">
            <div class="col-12">
                <div class="jumbotron">
                <div class="row row-cols-1 row-cols-md-3 g-4">
                <div class="col">
    <div class="card h-100">
      <img src="juego de mesa 1.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Collar</h5>
        <p class="card-text">collar DB Classic en oro blanco de 18kt con diamantes.</p>
      </div>
    </div>
  </div>
  <div class="col">
    <div class="card h-100">
      <img src="juego de mesa 2.jpg" class="card-img-top" alt="..."><br>
      <div class="card-body">
        <h5 class="card-title">Anillo</h5>
        <p class="card-text">Anillo Cameron con detalles de cristal.</p>
      </div>
    </div>
  </div>
  <div class="col">
    <div class="card h-100">
      <img src="juego de mesa 3.jpg" class="card-img-top" alt="...">
      <div class="card-body">
        <h5 class="card-title">Anillos de compromiso</h5>
        <p class="card-text">Anillo de compromiso con un diamante de corte brillante redondo y argolla en platino con diamantes.</p>
      </div>
    </div>
  </div>
                        </div>
                    </div>    
                </div>
            </div>
        </div>
    </div>
    <footer class="text-center">
        <hr>
        2022 &copy; jeweler's
    </footer>
    <script src="js/bootstrap.js"></script>
</body>
</html>
